...
import {discoverNewestArchives,parseCsvFile,haversineMiles,validCoordinate,headerIndex,normalizeNoaaRecord,filterNormalizeRecords,HISTORY_RADIUS_MILES,writeReportOutputs,summarizeRecords,sourceManifest,validateHistory} from '../scripts/hail-history-lib.mjs';
...
test('streaming CSV parser handles quoted commas, quotes, and embedded newlines',async()=>{
 const directory=await mkdtemp(join(tmpdir(),'peak-hail-csv-'));const path=join(directory,'test.csv');const rows:string[][]=[];
 try{await writeFile(path,'A,B,C\n1,"two, fields","line one\nline two"\n2,"said ""hail""",end\n');await parseCsvFile(path,(parsedRow:string[])=>rows.push(parsedRow));assert.deepEqual(rows,[['A','B','C'],['1','two, fields','line one\nline two'],['2','said "hail"','end']]);}finally{await rm(directory,{recursive:true,force:true});}
});
...
test('coordinate, year, event type, radius, and magnitude are validated conservatively',()=>{
 const index=headerIndex(header),valid=normalizeNoaaRecord(row(),index,archive);if(!valid)throw new Error('expected valid record');
 ...
 const blankMagnitude=normalizeNoaaRecord(row({MAGNITUDE:''}),index,archive);if(!blankMagnitude)throw new Error('expected blank-magnitude record');assert.equal(blankMagnitude.magnitude,null);
...
test('annual report counts, hail days, maximum magnitude, and thresholds are exact',()=>{
 const index=headerIndex(header),records=[normalizeNoaaRecord(row(),index,archive),normalizeNoaaRecord(row({EVENT_ID:'21',MAGNITUDE:'2.50'}),index,archive),normalizeNoaaRecord(row({EVENT_ID:'22',BEGIN_DATE_TIME:'02-JUN-25 12:00:00',MAGNITUDE:''}),index,archive)];
 const summary=summarizeRecords(records);assert.equal(summary.annual['2025'].report_count,3);assert.equal(summary.annual['2025'].hail_day_count,2);assert.equal(summary.overall.largest_reported_hail_inches,2.5);assert.equal(summary.overall.hail_report_count_1in_plus,2);assert.equal(summary.overall.hail_report_count_2in_plus,1);assert.equal(summary.annual['2024'].report_count,0);
});
test('summary consistency validation rejects changed totals and duplicate records',()=>{
 const record=normalizeNoaaRecord(row(),headerIndex(header),archive),summary=summarizeRecords([record]),manifest=sourceManifest(Array.from({length:10},(_,i)=>({year:2016+i,filename:`year-${2016+i}`,url:'https://www.ncei.noaa.gov/',correctionDate:'20260901'})),'2026-09-05T00:00:00Z');
 assert.equal(validateHistory([record],summary,manifest),true);assert.throws(()=>validateHistory([record],{...summary,overall:{...summary.overall,report_count:2}},manifest),/Summary/);assert.throws(()=>validateHistory([record,record],summarizeRecords([record,record]),manifest),/Duplicate/);
});
test('committed JSON, CSV, annual summary, and source manifest are consistent',async()=>{
 const records=JSON.parse(readFileSync('data/hail-history/hail-reports-2016-2025.json','utf8')),summary=JSON.parse(readFileSync('data/hail-history/hail-history-summary.json','utf8')),manifest=JSON.parse(readFileSync('data/hail-history/source-manifest.json','utf8'));
 assert.equal(validateHistory(records,summary,manifest),true);let csvRows=0;await parseCsvFile('data/hail-history/hail-reports-2016-2025.csv',(_row:string[],line:number)=>{if(line>1)csvRows++;});assert.equal(csvRows,records.length);assert.equal((Object.values(summary.annual) as Array<{report_count:number}>).reduce((total,year)=>total+year.report_count,0),summary.overall.report_count);
});
