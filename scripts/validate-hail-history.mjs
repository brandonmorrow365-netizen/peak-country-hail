...
for(const expression of ['publicHistory(','subsetSummary(','PUBLIC_HISTORY_YEARS','summary.report_count','summary.hail_day_count','summary.largest_reported_hail_inches','summary.hail_report_count_1in_plus','summary.hail_report_count_2in_plus','summary.closest_report_distance','summary.most_active_month'])assert.ok(page.includes(expression),`Public history page does not consume ${expression}`);
assert.ok(!page.includes('1,051 qualifying'),'Public page contains a duplicated hard-coded total');
console.log(`Reconciled ${csvCount} retained source reports across ${summary.overall.hail_day_count} hail days for 2016–2025; CSV, JSON, and source summary agree. The public page derives its five-year window from these records.`);
