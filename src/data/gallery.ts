...
export type PortfolioRepair = {
  ...
  title: string;
  summary: string;
  /** Public presentation facts are reviewed per repair; no customer information is published. */
...
export type PortfolioPair = {
  angle: number;
  before: PortfolioPhoto;
  after?: PortfolioPhoto;
};

/**
 * Keeps like-for-like views together in every public use of the portfolio.
 * A before view with no matching after is shown separately as honest context.
 */
export const portfolioPairs = (repair: PortfolioRepair): PortfolioPair[] => repair.photos
  .filter((photo) => photo.state === 'before')
  .map((before) => ({
    angle: before.angle,
    before,
    after: repair.photos.find((photo) => photo.state === 'after' && photo.angle === before.angle),
  }));
