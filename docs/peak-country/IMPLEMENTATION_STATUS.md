...
11. [x] Cross-site consistency + architecture/code consolidation (2026-09-14; replaced the animated canvas header with a compact accessible site header, normalized the footer and design system, removed abandoned visual stubs and their unused dependencies, corrected contact/process CTA language, documented the design system, and verified the result with Astro check, production build, 39 automated tests, 34-route private-preview QA, hail-history validation, hero/header validation, responsive visual inspection, source-of-truth scans, and `git diff --check`)
