...
- **Cloudflare Email Routing:** no routing rules or destination addresses; its DNS records are not configured
- **Google domain verification:** pending the unique TXT value supplied during Google Workspace setup
- **DKIM:** pending the selector and public key generated by Google Workspace
- **DMARC:** not configured during launch; add a considered policy after Google Workspace DKIM is active

The website contact form stores valid submissions in Cloudflare D1 when forms are enabled. It does not send mail as `@peakcountryhail.com`, so no additional sender was added to SPF. No Google passwords, API credentials, verification tokens, or DKIM keys belong in this repository.
